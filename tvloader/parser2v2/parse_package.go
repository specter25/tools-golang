...
func (parser *tvParser2_2) parsePairFromPackage2_2(tag string, value string) error {
	...
	case "PackageChecksum":
		...
		if parser.pkg.PackageChecksums == nil {
			parser.pkg.PackageChecksums = make([]spdx.Checksum2_2, 9)
		}
		switch subkey {
		case "SHA1":
			parser.pkg.PackageChecksums = append(parser.pkg.PackageChecksums, spdx.Checksum2_2{Algorithm: spdx.SHA1, Value: subvalue})
		case "SHA256":
			parser.pkg.PackageChecksums = append(parser.pkg.PackageChecksums, spdx.Checksum2_2{Algorithm: spdx.SHA256, Value: subvalue})
		case "MD5":
			parser.pkg.PackageChecksums = append(parser.pkg.PackageChecksums, spdx.Checksum2_2{Algorithm: spdx.MD5, Value: subvalue})
		default:
